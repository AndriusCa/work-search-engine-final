...
import defaultImage from "../../assets/default-job-bg.png";

export function EditJob() {
  const navigate = useNavigate();
  const { jobId } = useParams();
  const currentYear = new Date().getFullYear();
  const { role, jobTypes, steeringWheelSides } = useContext(GlobalContext);
    ...
    fetch("http://localhost:3001/api/jobs/" + jobId, {
      ...
      .then(data => {
        if (data.status === "ok") {
          ...
          setYear(job.year)
          setSteeringWheel(job.steeringWheel)
  ...
  function updateImage(e) {
    ...
    formData.append("job_image", e.target.files[0])

    fetch("http://localhost:3001/api/upload/job", {
      method: "POST",
      body: formData,
    })
      .then(res => res.json())
  ...
  function isValidForm() {
    ...
    const jobTypeMsg = jobTypeValidity()
    setjobTypeErr(jobTypeMsg)
    ...
    <div className="container">
      <div className="row">
        ...
        <form onSubmit={submitHandler} className="col-12 col-sm-8">
          ...
          <div className="row mb-3">
            <label className="col-12 col-md-4 form-label" htmlFor="jobType">
              Type
            </label>
            <div className="col-12 col-md-8">
              <select
                className={`form-select ${jobTypeErr ? "is-invalid" : ""}`}
              ...
              <div className="invalid-feedback">{jobTypeErr}</div>
            </div>
          </div>
          <div className="row mb-3">
